...
Analisando a fisionomia de $f(t)$, o chute inicial $t_{0}$ não pode ter valores negativos pois ele está contido dentre raizes quadradas e nem pode ser nulo pois ele tem papel de divisor em alguns termos. Analisando o provável valor máximo de t, percebe-se que  Ao utilizar o ponto fixo gerado pelo algoritmo desta questão (Figura~\ref{fig:p2q6g1}) como entrada de $f(t^{*})$ foi obtido como resultado $f(t^{*}) = 0$, o que indica que a raiz encontrada está correta.
...
\begin{figure}[H]
	\centering
	\includegraphics[width=1\textwidth]{p2q8.png}
...
\textbf{Resolução:} O algoritmo implementado para resolver esta questão também está presente no arquivo "$\textit{parte2/questao78910.f90}$"
